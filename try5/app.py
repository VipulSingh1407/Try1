...
def chatbot_response(user_data):
    ...
    try:
        ...
        scores = {
        'kapha_score': kapha_percentage,
        'pitta_score': pitta_percentage,
        'vata_score': vata_percentage
    }
        
        # Return the result along with dosha information
        result_message = f"ChatBot says: based on your responses, you have a {dosha} dosha constitution. You can follow the given tips for a healthier lifestyle."
        
        # Include dosha scores in the response
        #result_message += f"\nPitta Score: {pitta_percentage}%"
       # result_message += f"\nVata Score: {vata_percentage}%"
        
        return jsonify({'user_response':result_message, 'scores':scores})
